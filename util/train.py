...
class Train(object):
    def __init__(self, agent):
        self.agent = agent
        self.args = args = self.agent.parse_args()
        get_logger(self.agent.algorithm, args.env, args.env_type)
        ...
        U.main_logger.info("Replay Buffer Max Size: {}B".format(pretty_num(args.replay_buffer_size *
                                                                           (84 * 84 * 4 * 2 + 8), True)))
        self.sess = self.agent.make_session()
        self.sess.__enter__()
        self.agent.setup(self.ep.action_num, self.replay)
        self.train_epi = 0
        self.max_reward = self.agent.score
    ...
    def train(self):
        ...
        with tqdm(total=train_step, desc="Train", ascii=True) as t:
            while t.n < train_step:
                action = self.agent.take_action(obs, self.eps[0].get(self.train_epi * train_step + t.n))
    ...
    def test(self):
        ...
        with tqdm(total=test_step, desc="Evaluation", ascii=True) as t:
            while t.n < test_step:
                action = self.agent.take_action(obs, self.eps[1])
                self.ep.step(action)
                obs, info = self.ep.auto_reset()
                t.update(self.ep.size)
        total_epi = sum(len(info[i]['rewards']) for i in range(self.ep.size))
